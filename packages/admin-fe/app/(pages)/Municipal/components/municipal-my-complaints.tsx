...
export function MunicipalMyComplaints() {
  ...
  const fetchMyComplaints = async () => {
    ...
    try {
      ...
      // Use municipal admin specific endpoint
      const res = await fetch(`/api/municipal-admin/my-complaints?page=${pagination.currentPage}&limit=${pagination.itemsPerPage}`, {
  ...
  const canUpdateStatus = (complaint: Complaint): boolean => {
    ...
    // Municipal admin can update complaints managed by them
  ...
  const updateComplaintStatus = async (complaintId: string, newStatus: string) => {
    try {
      ...
      const isEscalation = newStatus === 'ESCALATED_TO_STATE_LEVEL'
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-gray-500 mt-1">Manage complaints in your municipality</p>
        </div>
        <Button 
          variant="outline" 
                  ...
                  {selectedComplaint.status !== 'ESCALATED_TO_STATE_LEVEL' && (
                    <div className="mt-3">
                      <Button
                        ...
                        onClick={() => updateComplaintStatus(selectedComplaint.id, 'ESCALATED_TO_STATE_LEVEL')}
                      >
